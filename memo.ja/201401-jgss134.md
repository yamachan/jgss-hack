...
ちょっと量が多いので、plugins.js と rpg_core.js で前編としておきます。
...
CacheMap.update メソッドが呼び出されると、この寿命のチェックがおこなわれ、不要になったリソースが free されるようです。これがどこから呼ばれているのか、後で探してみましょう。
...
リソースに寿命の概念が導入されたので、画像 Bitmap クラスに、touch メソッドが追加されています。
...
そして以下の部分で、このハンドラを組み込んだり外したり、利用するよう拡張していますね。互換性を維持するための苦労が察せられます。
...
また Tilemap を継承した ShaderTilemap クラスが新規に追加されています。
