...
```js
MyWindow.prototype.initialize = function(width) {
    this._windowWidth = width;
    Window_HorzCommand.prototype.initialize.call(this, 0, 0);
};
...
Scene_Shop.prototype.createMyWindow = function() {
  ...
  this._myWindow = new MyWindow(456);
...
とりあえず左上に追加した選択リストが表示されているのがわかります。現時点では何の意味もないというか、単に邪魔なんですけどねw

## カテゴリ選択ウィンドウの位置合わせ

まずは位置を直さないと駄目ですね。先ほど定義した initialize の y 位置を修正しましょう。引数で y を指定できるように改良し…
...
さいしょからずっと表示されているのは変ですので、作成直後は hide() でかくしておきましょう。
...
しかしまだ、その先の商品リストも表示されませんし、また MyWindow から元のコマンドリストにも戻れません。ゲームを強制終了するしかなくなります。
...
MyWindow で選択した my_select に合致するものだけをリストするように変更します。
...
それぞれ順に対処していきましょう。商品リストでのキャンセル処理は以下ですが
...
これの buy の処理を変更し、単純に商品リストを再表示させ選択するだけにしました。
