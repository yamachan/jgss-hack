...
わりと王道のサイズ管理をしています。まず、管理対象のイメージごとに最後に使用した時間(touch)を保持しておきます。そして add 関数で新しいイメージが追加された時、イメージの総サイズが上限を越えていれば、touch の値をみて古いものから破棄しています。
...
ちなみに私だったら、ちょっと無駄は出ますがシンプルに以下のように add に reserve の機能をマージしちゃいますねw
...
あと Date.now() を毎回呼ぶのもアレなので、内部で常に増加している値、例えば fps の値あたりを利用するかも、です。あ、話題がそれすぎですかね…
...
ver1.5.0 から rpg_core.js に RequestQueue クラスが追加されています。
