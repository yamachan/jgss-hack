...
また VX Ace のページですが [押せる岩を作る](https://tkool.jp/products/rpgvxace/lecture/006_003q) のイベント設定は MV でも有効です。 この押せる岩などが画面にあれば、岩で視界を塞ぐなど、更なるパズル要素が実現できそうです。
...
障害物の確認はプレイヤー用の歩行可能かどうかのロジックを利用しています。 ですのでプレイヤーが通れるところは、敵の視線も通る、ということになります。
...
自動で動くように設定したイベントですが、RPGツクールMVでは 「移動するときに、全てのイベントとぶつかり判定がある」 という仕様になっているようです。 プライオリティが「通常キャラと同じ」ものはともかく、通常キャラの下や上に配置したイベントも通ることができません。

さきほど説明した 「ポケモンっぽい敵キャラ」 の前に、プライオリティを通常キャラと異なって設定したイベントを置いた場合、プレイヤーは通行できるので、敵キャラの視線も通ります。 しかし敵キャラが追跡モードになると、その部分は通れませんので、それを避けて追いかけてきます。 もし配置したイベントに隙間がないとプレイヤーに追いつけなくなります。

この場合、 「プライオリティを通常キャラと異なって設定したイベント」 それ自体の通り抜けフラグを ON にしてください。 そうすれば敵キャラもそのイベントを通れるようになります。
...
| --- | --- | --- | --- |
| ver1.03 | 2016/07/23 | なし | プライオリティが通常キャラと異なる場合に視線が通らないバグを修正 |
| [ver1.02](archive/RTK_EnemySight_v1.02.js) | 2016/07/20 | なし | 壁などの障害物が視界を遮るよう改良<br>フォロワーも見つかるオプションを追加 |
| [ver1.01](archive/RTK_EnemySight_v1.01.js) | 2016/07/16 | なし | ロジック修正 |
| [ver1.00](archive/RTK_EnemySight_v1.00.js) | 2016/07/02 | なし | 公開 |
