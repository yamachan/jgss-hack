...
自動で動くように設定したイベントですが、理由はわかりませんが RPGツクールMVでは 「移動するときに、全てのイベントとぶつかり判定がある」 という仕様になっているようです。 プライオリティが「通常キャラと同じ」ものはともかく、通常キャラの下や上に配置したイベントも通ることができません。

これを簡単に確かめるには、プレイヤーを追いかけるイベントをマップに追加し、そのまわりをぐるりと、プライオリティが通常キャラと異なるイベントで囲んでみてください。 プレイヤーはその中に自由に出入りできますが、プレイヤーを追いかけるイベントはその中から出てこれないはずです。
...
マップの見た目を良くするためや、シナリオの進行によって見た目を変えるためなど、イベントを背景の一部のように使っている場合には、この点に注意が必要です。
